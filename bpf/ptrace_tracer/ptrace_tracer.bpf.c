...
struct {
    ...
    __type(key, u32);
    __type(value, struct ptrace_event_t);
} tmp_event_map SEC(".maps");

struct {
	__uint(type, BPF_MAP_TYPE_RINGBUF);
	__uint(max_entries, 1 << 24);
} events_ptrace SEC(".maps");
...
} start_events_ptrace SEC(".maps");

const struct ptrace_event_t *unused __attribute__((unused));

SEC("tracepoint/syscalls/sys_enter_ptrace")
int handle_enter_ptrace( struct trace_event_raw_sys_enter *ctx){

  u64 pid_tgid = bpf_get_current_pid_tgid();
  u32 key = 0;

  struct ptrace_event_t *event = bpf_map_lookup_elem(&tmp_event_map, &key);
  if (!event)
      return 0;
  ...
  event->request = (long) ctx->args[0];
  event->pid_ptrace = (long) ctx->args[1];
  event->addr = (long) ctx->args[2];
  event->data = (long) ctx->args[3];

  event->timestamp_ns = bpf_ktime_get_ns();

  // finally, persist it to start_events
  bpf_map_update_elem(&start_events_ptrace, &pid_tgid, event, BPF_ANY);
  return 0;

}

SEC("tracepoint/syscalls/sys_exit_ptrace")
int handle_exit_ptrace(struct trace_event_raw_sys_exit *ctx){
  
  struct ptrace_event_t *event;

  u64 pid_tgid = bpf_get_current_pid_tgid();

  event = bpf_map_lookup_elem(&start_events_ptrace,&pid_tgid);
  if (!event)
    return 0;

  struct ptrace_event_t *final_event;

  final_event = bpf_ringbuf_reserve(&events_ptrace,sizeof(struct ptrace_event_t),0);

  if(!final_event) return 0;

  long ret = ctx->ret;

  __builtin_memcpy(final_event, event, sizeof(struct ptrace_event_t));
  ...
  bpf_map_delete_elem(&start_events_ptrace,&pid_tgid);
