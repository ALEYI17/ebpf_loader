...
u64 pid_tgid = bpf_get_current_pid_tgid();
  ...
  bpf_map_update_elem(&exec_start_events, &pid_tgid, event, BPF_ANY);
  return 0;  
}
...
int handle_exit_execve(struct trace_event_raw_sys_exit *ctx){
  ...
  event = bpf_map_lookup_elem(&exec_start_events,&pid_tgid);
  if (!event)
    return 0;

  struct trace_syscall_event *final_event;

  final_event = bpf_ringbuf_reserve(&exec_events,sizeof(struct trace_syscall_event),0);
  ...
  bpf_map_delete_elem(&exec_start_events,&pid_tgid);
