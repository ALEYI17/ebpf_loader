...
struct {
    ...
    __type(key, u32);
    __type(value, struct trace_syscall_event);
...
u64 pid_tgid = bpf_get_current_pid_tgid();
  ...
  bpf_map_update_elem(&chmod_start_events, &pid_tgid, event, BPF_ANY);
  return 0;  
}
...
int handle_exit_chmod(struct trace_event_raw_sys_exit *ctx){
  ...
  event = bpf_map_lookup_elem(&chmod_start_events,&pid_tgid);
  if (!event)
    return 0;

  struct trace_syscall_event *final_event;

  final_event = bpf_ringbuf_reserve(&chmod_events,sizeof(struct trace_syscall_event),0);

  if(!final_event) return 0;

  long ret = ctx->ret;

  __builtin_memcpy(final_event, event, sizeof(struct trace_syscall_event));
  ...
  bpf_map_delete_elem(&chmod_start_events,&pid_tgid);
