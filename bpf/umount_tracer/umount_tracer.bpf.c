...
} start_events_umount SEC(".maps");

const struct mount_event_t *unused __attribute__((unused));


SEC("tracepoint/syscalls/sys_enter_umount")
int handle_enter_umount( struct trace_event_raw_sys_enter *ctx){
  ...
  bpf_probe_read_user_str(event->dir_name, sizeof(event->dev_name), (const char *)ctx->args[0]);
  event->flags = (unsigned long) ctx->args[1];

  event->timestamp_ns = bpf_ktime_get_ns();

  // finally, persist it to start_events
  bpf_map_update_elem(&start_events_umount, &pid_tgid, event, BPF_ANY);
  return 0;

}

SEC("tracepoint/syscalls/sys_exit_mount")
int handle_exit_umount(struct trace_event_raw_sys_exit *ctx){
  
  struct mount_event_t *event;

  u64 pid_tgid = bpf_get_current_pid_tgid();

  event = bpf_map_lookup_elem(&start_events_umount,&pid_tgid);
  if (!event)
    return 0;

  struct mount_event_t *final_event;

  final_event = bpf_ringbuf_reserve(&events_umount,sizeof(struct mount_event_t),0);
  ...
  bpf_map_delete_elem(&start_events_umount,&pid_tgid);
